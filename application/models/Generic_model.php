    ...
    */
    ...
    public function admin_routine($section_parameters = FALSE, $content = FALSE, $id = FALSE)
    {
        ...
        */
        ...
        $course_item = array('id' => strval(99), 'item' => 'Courses', 'role' => strval(1), 'level' => strval(1), 'icon' => 'fa-solid fa-book', 'link' => '');
        array_push($this->data['items_menu'], $course_item);
        $courses = $this->read_all_records('courses');
        foreach ($courses as $course) {
            array_push(
                $this->data['items_menu'],
                array('id' => strval($course['id']), 'item' => $course['course'], 'role' => strval(1), 'level' => strval(2), 'icon' => 'fa-solid fa-check', 'link' => 'admin/courses_admin/course/' . strtolower($course['course']))
            );
        }
